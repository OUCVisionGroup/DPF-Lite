...
def disp_to_depth(disp, min_depth, max_depth):
    return disp*(max_depth-min_depth) + min_depth
...
class CurriculumScheduler:
    def __init__(self):
        # self.stages = [
        #     {'epochs': 10, 'params': range(3)},
        #     {'epochs': 20, 'params': range(6)},
        #     {'epochs': 30, 'params': range(8)},
        #     {'epochs': -1, 'params': range(10)}
        # ]
        self.stages = [
            {'epochs': 10, 'params': [2, 3]},
            {'epochs': 20, 'params': [2, 3, 5, 6]},
            {'epochs': 30, 'params': range(8)},
            {'epochs': -1, 'params': range(10)}
        ]
    ...
    def update(self):
        ...
        if self.current_stage < len(self.stages) - 1:
            if self.epoch_counter >= self.stages[self.current_stage]['epochs']:
...
class NYU_Dataset(Dataset):
    def __init__(self, device, nyu_path=r'./NYU-Depth-V2/Img',
                 water_path=r'./UIEB/train/raw', depth_scale=r'./NYU-Depth-V2/depth_scale.txt',
                 depth_path=r'./NYU-Depth-V2/Depth', Image_size=256, isTrain=True):
        ...
        self.device = device
        print("Total image pairs:", len(self.onland_list))
    ...
    def __getitem__(self, index):
        ...
        data_depth = 1 - np.float32(data_depth) / 255.0
        non_zero_depths = data_depth[data_depth > 0]
        min_non_zero = np.min(non_zero_depths)
        data_depth[data_depth == 0] = min_non_zero

        max_depth = self.depth_scale[data_onland_path][0]
        min_depth = self.depth_scale[data_onland_path][1]
        abs_depths = disp_to_depth(data_depth, min_depth, max_depth)

        available_params = self.curriculum.get_available_params()
        category = np.random.choice(available_params)
        ...
        abs_depths = torch.from_numpy(abs_depths).unsqueeze(0)
        abs_depths = abs_depths.to(self.device, dtype=torch.float32)
        rel_depths = torch.from_numpy(data_depth).unsqueeze(0)
        rel_depths = rel_depths.to(self.device, dtype=torch.float32)

        return data_onland, degraded_img, rel_depths, abs_depths, B, beta_D, beta_B, pre_B
