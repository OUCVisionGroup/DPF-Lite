...
class DepthDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        for i in range(2, -1, -1):
            ...
            # if i in self.scales:
            #     self.outputs[("disp", i)] = upsample(self.convs[("dispconv", i)](x), mode='bilinear')
            if i == 0:
                self.outputs = self.sigmoid(upsample(self.convs[("dispconv", i)](x), mode='bilinear'))
                #self.outputs = F.relu(upsample(self.convs[("dispconv", i)](x), mode='bilinear'))
